...
const CodeChef = () => {
  ...
  const fetchSolvedQuestionsChef = async () => {
    if (!username.trim()) {
    ...
    try {
      const response = await axios.get(`http://localhost:5002/${username}/solved`);
  ...
  return (
    <div className="bg-white min-h-screen flex flex-col justify-center items-center p-6">
      <div className="bg-gray-600 p-6 rounded-xl shadow-2xl max-w-md w-full text-center">
        ...
        <h2 className="text-xl font-bold text-white mb-4">CodeChef Stats</h2>
        ...
        <div className="mb-4">
          ...
          />
          <button
            className="w-full mt-2 bg-gray-900 text-white py-2 rounded-lg hover:bg-00"
            onClick={fetchSolvedQuestionsChef}
            disabled={loading}
          >
